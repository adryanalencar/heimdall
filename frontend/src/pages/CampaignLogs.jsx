...
const CampaignLogs = () => {
  ...
  const getStatusBadge = (status) => {
    ...
    if (status === 'delivered') classes = "bg-[#dcf8c6] text-[#075e54] border border-[#25d366]";
    else if (status === 'read') classes = "bg-purple-100 text-purple-700 border border-purple-200";
    else if (status === 'sent') classes = "bg-blue-100 text-blue-700 border border-blue-200";
    else if (status === 'failed') classes = "bg-red-100 text-red-700 border border-red-200";
    
    return (
      <span className={`px-2 py-1 rounded-full text-xs font-semibold ${classes}`}>
        {t(`logs.statuses.${status}`) || status}
      </span>
    );
  };
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-[#e9edef] text-gray-500 text-sm uppercase">
                  <th className="px-6 py-4 font-semibold">{t('logs.recipient')}</th>
                  <th className="px-6 py-4 font-semibold">{t('logs.status')}</th>
                  <th className="px-6 py-4 font-semibold">{t('logs.timestamp')}</th>
                  <th className="px-6 py-4 font-semibold">{t('logs.error')}</th>
                </tr>
              </thead>
              <tbody className="text-gray-700 text-sm">
                {loading ? (
                   ...
                   <tr>
                     <td colSpan="4" className="px-6 py-8 text-center text-gray-500">{t('logs.noLogs')}</td>
                   </tr>
                ) : (
                  filteredLogs.map((log, index) => (
                    <tr key={index} className="border-b border-[#e9edef] hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-red-500 max-w-xs truncate" title={log.error}>
                        {log.error || '-'}
                      </td>
                    </tr>
                  ))
